...
const MarketShareTreemap = ({ data, secondsData }) => {
  ...
  const getTitle = () => {
    ...
    if (selectedCategory)
      return `${selectedCategory} Brands (${unit}, Station: ${selectedStation})`;
    if (selectedSector)
      return `${selectedSector} Categories (${unit}, Station: ${selectedStation})`;
    return `Market Share by Sector (${unit}, Station: ${selectedStation})`;
  };

  // Prepare treemap data
  const treemapData = useMemo(() => {
    const currentData = usespots ? data : secondsData;
    const stationData = currentData.stations.find(
      (s) => s.station === selectedStation
    );
    ...
    if (currentLevel === "sectors") {
      ...
      const sector = stationData.sectors.find((s) => s.name === selectedSector);
      const category = sector?.categories?.find(
    ...
    <Card className="p-0 gap-0 w-full">
      <CardHeader className="p-4 flex flex-row items-center justify-between">
        <div className="flex flex-col">
          ...
          <CardDescription>
            View market share distribution across sectors, categories, brands
          </CardDescription>
        </div>
        <div className="flex flex-row items-center justify-between gap-4">
          ...
          {(selectedSector || selectedCategory) && (
            ...
            >
              ...
              Back to {selectedCategory ? "Categories" : "Sectors"}
            </button>
          )}
        </div>
      </CardHeader>
